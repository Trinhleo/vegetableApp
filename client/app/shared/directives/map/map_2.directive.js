        ...
        function link(scope, element, attrs) {
            ...
            function setMarker(events) {
                ...
                events.forEach(function (evt) {
                    ...
                    var hostProfileLink =scope.myId.toString()===evt.user._id.toString()?'#/me':'#/users/' + evt.user._id;
                    var detailsLink = '#/events/' + evt._id;
                    var editLink = detailsLink.concat('/edit');
                    var address = evt.address;
                    var userHostAvatar = evt.userHostProfileImageURL ? evt.userHostProfileImageURL : evt.userHost.profileImageURL;
                    var contentString = '<div id="iw-container"><div class="iw-title"> <a style="color:#f5f5f5" href="' + detailsLink + '">' + name + '</a></div>'
                        ...
                        + '<hr>'
                        + '<h5>Hosted by:<a href="' + hostProfileLink + '"> <img style="height:36px; width:36px; border-radius:50%" src="' + userHostAvatar + '" /> <strong style ="color: #000099">' + evt.userHost.firstName + ' ' + evt.userHost.lastName
                        + '</a></div>'
                        + '</strong></h5>'
